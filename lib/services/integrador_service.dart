...
class IntegradorService {
  ...
  Future<List<List<dynamic>>> _parseXlsxManual(List<int> bytes) async {
    try {
      ...
      final cellRegex = RegExp(r'<c[^>]*r="([A-Z]+)(\d+)"[^>]*(?:t="([^"]*)")?[^>]*>(?:<v>([^<]*)</v>)?</c>');
      ...
      for (final rowMatch in rowRegex.allMatches(sheetXml)) {
        ...
        for (final cellMatch in cellRegex.allMatches(rowContent)) {
          final colStr = cellMatch.group(1) ?? '';
          final type = cellMatch.group(3);
          final value = cellMatch.group(4);
          ...
          if (value == null || value.isEmpty) {
            ...
            row.add(index != null && index < sharedStrings.length ? sharedStrings[index] : value);
          } else {
            // Número o texto
  ...
  Future<Map<String, int>> _procesarFilas({
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      // CAPÍTULO: código contiene #
      if (codigo.contains('#')) {
        capituloActualId = await _crearCapitulo(
          presupuestoId: presupuestoId,
          codigo: codigo,
